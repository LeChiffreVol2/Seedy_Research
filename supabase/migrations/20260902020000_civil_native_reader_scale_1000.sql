...
-- Native-reader scale gate: one Thai discovery page must remain one bounded
-- database page even after the licensed cohort grows past 1,000 papers.
create index if not exists civil_source_catalog_native_feed_idx
on public.civil_source_catalog (
  (case
    when evidence_status in ('extracted', 'indexed') then 0
    when evidence_status = 'metadata_only' then 1
    else 2
  end),
  published_at desc,
  id
...
create index if not exists civil_source_catalog_provider_native_feed_idx
on public.civil_source_catalog (
  provider,
  (case
    when evidence_status in ('extracted', 'indexed') then 0
...
-- Retain the exact asset/page access path as an explicit scale contract. The
-- unique constraint currently covers the same columns; this named index keeps
-- older deployments and migration-only environments aligned.
...
is 'Bounded rights-safe catalog page with optional native-first ordering; stored abstracts never cross the response boundary.';
