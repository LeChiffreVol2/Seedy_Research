...
-- Let each trigram index scan stop as soon as its bounded candidate budget is
-- filled. Sorting every match before LIMIT defeated the bound for common terms.
create or replace function public.search_civil_sections_lexical_v2(
  search_query text, match_count int default 20, filter_disc text default null,
...
create or replace function public.search_civil_chunks_lexical_v2(
  search_query text, match_count int default 8, filter_disc text default null,
  filter_document_ids text[] default null, filter_section_ids text[] default null,
  filter_collection text default null
)
returns table (
