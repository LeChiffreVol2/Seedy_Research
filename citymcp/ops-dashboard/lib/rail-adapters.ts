...
type RailOverviewOptions = {
  allowReadModel?: boolean;
};
...
const DRT_DATASTORE_SEARCH_URL = "https://drt.gdcatalog.go.th/api/3/action/datastore_search";
...
async function readOfficialDrtRows(url: string, kind: "crossing" | "plan"): Promise<{ rows: JsonObject[]; source: "CKAN datastore" | "CSV"; lastModified: string | null }> {
  const errors: string[] = [];
  const datastoreResourceId = datastoreResourceForKnownCsv(url, kind);
  if (datastoreResourceId) {
    try {
      return { rows: await fetchDatastoreRows(datastoreResourceId), source: "CKAN datastore", lastModified: null };
    } catch (error) {
      errors.push(`CKAN datastore: ${error instanceof Error ? error.message : "unknown error"}`);
    }
  }
  try {
    const payload = await fetchText(url);
    return { rows: parseCsv(payload.text), source: "CSV", lastModified: payload.lastModified };
  } catch (error) {
    errors.push(`CSV: ${error instanceof Error ? error.message : "unknown error"}`);
...
function shouldReadRailBaselineFromSupabase(allowReadModel = true) {
  return Boolean(
    allowReadModel &&
      process.env.SUPABASE_URL &&
      process.env.SUPABASE_SERVICE_KEY &&
      (process.env.VERCEL || process.env.SMART_CITY_RAIL_READ_MODEL_FIRST === "true"),
  );
}
...
async function readSupabaseSourceHealth(params: {
  ...
  if (!shouldReadRailBaselineFromSupabase(params.allowReadModel)) return null;
  try {
...
async function previousOfficialHealth(params: {
  sourceId: string;
  name: string;
  provider: string;
  attemptedAt: string;
  ...
  const previous = await readSupabaseSourceHealth({ ...params, allowReadModel: true });
  ...
  return sourceHealth({
    ...
    message: `${params.failureMessage}; retaining previous official baseline from ${previous.lastSuccessAt}.`,
    dataClass: previous.dataClass ?? params.fallbackDataClass,
    refreshPolicy: previous.refreshPolicy ?? params.fallbackRefreshPolicy,
    lastModified: previous.lastModified,
    upstreamCadence: previous.upstreamCadence ?? params.fallbackUpstreamCadence,
  });
}

async function readSupabaseDrtCrossings(attemptedAt: string, allowReadModel: boolean): Promise<{ crossings: SmartCityAsset[]; health: SourceHealth } | null> {
  if (!shouldReadRailBaselineFromSupabase(allowReadModel)) return null;
...
async function readDrtCrossingPlan(url: string, attemptedAt: string, allowReadModel: boolean): Promise<{ rows: JsonObject[]; bySurvey: DrtPlanBySurvey; health: SourceHealth }> {
  ...
  try {
    const payload = await readOfficialDrtRows(url, "plan");
    ...
    return {
      ...
      health: sourceHealth({
        ...
        message:
          rows.length > 0
            ? `Official DRT crossing improvement plan loaded from ${payload.source}.`
            : "DRT plan source returned no rows.",
        dataClass: "official_baseline",
        refreshPolicy: payload.source === "CKAN datastore" ? "official DRT CKAN datastore pull" : "official DRT CSV pull",
        lastModified: payload.lastModified,
        upstreamCadence: "official dataset publication cadence",
      }),
    };
  } catch (error) {
    const failureMessage = `DRT crossing plan CSV failed: ${error instanceof Error ? error.message : "unknown error"}`;
    const previousHealth = await previousOfficialHealth({
      sourceId,
      name,
      provider,
      attemptedAt,
      fallbackDataClass: "official_baseline",
      fallbackRefreshPolicy: "official DRT CSV pull",
      fallbackUpstreamCadence: "official dataset publication cadence",
      failureMessage,
      latencyMs: Date.now() - started,
    });
...
async function readDrtCrossings(url: string, attemptedAt: string, planBySurvey: DrtPlanBySurvey, allowReadModel: boolean): Promise<{ crossings: SmartCityAsset[]; health: SourceHealth }> {
  const sourceId = "drt-crossing-csv";
  const name = "DRT official rail crossing geometry";
  const provider = "Department of Rail Transport / data.go.th";
  const started = Date.now();
  const readModel = await readSupabaseDrtCrossings(attemptedAt, allowReadModel);
  if (readModel) return readModel;

  try {
    const payload = await readOfficialDrtRows(url, "crossing");
    const rows = payload.rows;
    ...
    return {
      ...
      health: sourceHealth({
        ...
        message:
          crossings.length > 0
            ? `Official DRT crossing geometry loaded from ${payload.source}; ${
    ...
    const failureMessage = `DRT crossing CSV failed: ${error instanceof Error ? error.message : "unknown error"}`;
    const previousHealth = await previousOfficialHealth({
      sourceId,
      name,
      provider,
...
async function readDrtCrossingAccidents(url: string, attemptedAt: string, allowReadModel: boolean): Promise<{ events: SmartCityEvent[]; health: SourceHealth }> {
  ...
  try {
    ...
    const failureMessage = `DRT accident CSV failed: ${error instanceof Error ? error.message : "unknown error"}`;
    const previousHealth = await previousOfficialHealth({
      sourceId,
      name,
      provider,
...
async function fetchRailSafetyOverview(options: RailOverviewOptions = {}): Promise<RailOverview> {
  loadOpsEnv();
  const allowReadModel = options.allowReadModel !== false;
  ...
  const [news, drtPlan, drtAccidents, crossingFeed, osirisContext] = await Promise.all([
    ...
    readDrtCrossingPlan(drtPlanUrl, attemptedAt, allowReadModel),
    readDrtCrossingAccidents(drtAccidentsUrl, attemptedAt, allowReadModel),
  ...
  const drtCrossings = await readDrtCrossings(drtCrossingUrl, attemptedAt, drtPlan.bySurvey, allowReadModel);
...
export async function getRailSafetyOverview(options: RailOverviewOptions = {}): Promise<RailOverview> {
  loadOpsEnv();
  if (options.allowReadModel === false) return fetchRailSafetyOverview(options);
