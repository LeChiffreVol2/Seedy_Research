...
import { buildRailCasesFromRealData, getRailSafetyOverview } from "./rail-adapters";
...
function shouldThrowReadModelFailure(): boolean {
  loadOpsEnv();
...
function toIngestRunSourceRow(runId: string, sourceIdSet: Set<string>, health: SourceHealth): IngestRunSourceRow | null {
  if (!sourceIdSet.has(health.sourceId)) return null;
  const status: IngestRunSourceRow["status"] =
    health.status === "ok"
      ? "succeeded"
  ...
  return {
    ...
    source_id: health.sourceId,
    status,
    attempted_at: health.lastAttemptAt || new Date().toISOString(),
    last_success_at: health.lastSuccessAt,
    record_count: health.recordCount,
    stale_count: 0,
    latency_ms: health.latencyMs,
    message: health.message || null,
    error_message: status === "failed" ? health.message || health.status : null,
  };
}
...
export function sourceIdsEligibleForMissingRowStaleMarking(health: Array<Pick<SourceHealth, "sourceId" | "status">>): string[] {
  return health.filter((item) => item.status === "ok" || item.status === "stale").map((item) => item.sourceId);
}
...
function eligibleSourceIds(health: SourceHealth[], target: "layers" | "insights"): Set<string> {
  return new Set(
    health
      .filter((item) => {
        const explicit = target === "layers" ? item.isEligibleForLayers : item.isEligibleForInsights;
        if (explicit !== undefined) return explicit;
        return Boolean(
          item.lastSuccessAt &&
            (item.status === "ok" || item.status === "degraded") &&
            item.dataClass !== "stale" &&
            item.dataClass !== "needs_config",
        );
      })
...
function sourceIsEligible(sourceId: string | null | undefined, eligibleIds: Set<string>): boolean {
  return Boolean(sourceId && eligibleIds.has(sourceId));
}

function sourceListIsEligible(sourceIds: string[] | undefined, eligibleIds: Set<string>): boolean {
  return Array.isArray(sourceIds) && sourceIds.length > 0 && sourceIds.some((sourceId) => eligibleIds.has(sourceId));
}
...
export async function buildSpatialCoreSnapshot(): Promise<SpatialSnapshot> {
  const [overview, railOverview] = await Promise.all([getThailandTransportOverview(), getRailSafetyOverview({ allowReadModel: false })]);
...
export async function refreshSpatialCoreReadModel(): Promise<{
  ...
  try {
    ...
    counts.ingestRunSources = await upsertRows(
      "smart_city_ingest_run_sources",
      health
        .map((item) => toIngestRunSourceRow(runId, sourceIdSet, item))
        .filter((item): item is IngestRunSourceRow => item !== null),
      "run_id,source_id",
    );
    ...
    const succeededSourceIds = sourceIdsEligibleForMissingRowStaleMarking(health);
    const degradedSourceIds = health
    ...
    await rpcValue<Record<string, number>>("smart_city_finish_ingest_run", {
      p_run_id: runId,
      p_status:
        failedSourceIds.length > 0 || degradedSourceIds.length > 0
          ? succeededSourceIds.length > 0
            ? "partial"
            : "failed"
          : "succeeded",
...
async function readSupabaseOverview(): Promise<OpsOverview | null> {
  ...
  const layerEligibleSourceIds = eligibleSourceIds(sourceState.health, "layers");
  ...
  const events = eventRows
    .filter((row) => sourceIsEligible(row.source_id, layerEligibleSourceIds))
    .map(fromEventRow)
  ...
  const hotspots = hotspotRows
    .filter((row) => sourceListIsEligible(row.source_ids, layerEligibleSourceIds))
    .map(fromHotspotRow)
    .filter((hotspot): hotspot is SmartCityHotspot => hotspot !== null);
...
async function readSupabaseOntology(filters: { type?: string | null; bbox?: Bbox | null; updatedSince?: string | null } = {}): Promise<OntologyReadModel | null> {
  ...
  const [objectRows, linkRows] = await Promise.all([
    ...
    readRows<LinkRow>("smart_city_links", new URLSearchParams({ select: "*", is_stale: "eq.false", limit: "5000" })),
  ]);
...
async function readSupabaseInsights(filters: InsightFilter = {}): Promise<{ generatedAt: string; insights: SmartCityInsight[] } | null> {
  ...
  if (!spatialReadModelConfigured()) return null;
  const limit = Math.max(0, Math.min(200, filters.limit ?? 50));
  const ontology = await readSupabaseOntology({ bbox: filters.bbox });
  const eligibleObjectIds = new Set(ontology?.objects.map((object) => object.id) ?? []);
  const params = new URLSearchParams({
    select: "*",
    domain: "eq.transport",
    is_stale: "eq.false",
    order: "last_seen_at.desc,risk_before.desc,confidence.desc",
    limit: "200",
  });
  const rows = await readRows<InsightRow>("smart_city_insights", params);
  if (rows.length === 0) return { generatedAt: new Date().toISOString(), insights: [] };
  const insights = rows
    .map(fromInsightRow)
    .filter(
      (insight) =>
        eligibleObjectIds.has(insight.objectId) &&
        (insight.sourceObjectIds.length === 0 || insight.sourceObjectIds.some((objectId) => eligibleObjectIds.has(objectId))),
    )
    .slice(0, limit);
  return {
...
export async function getReadModelOverview(): Promise<OpsOverview & { readModel?: "supabase" | "request_time_adapter" }> {
  try {
    ...
    if (shouldThrowReadModelFailure()) throw error;
    // Fall through to request-time adapter for local or not-yet-migrated environments.
  }
  if (shouldThrowReadModelFailure()) throw new Error("Supabase smart_city overview read model returned no usable rows.");
  const overview = await getThailandTransportOverview();
  return { ...overview, readModel: "request_time_adapter" };
}

function sourceLooksRail(source: SmartCitySource): boolean {
  return /rail|srt|drt|crossing/i.test(`${source.id} ${source.category} ${source.name} ${source.provider}`);
}

export async function getReadModelRailOverview(): Promise<RailOverview & { readModel?: "supabase" | "request_time_adapter" }> {
  const overview = await getReadModelOverview();
  if (overview.readModel !== "supabase") {
    const railOverview = await getRailSafetyOverview();
    return { ...railOverview, readModel: "request_time_adapter" };
  }

  const crossings = overview.assets.filter((asset) => asset.assetType === "rail_crossing" || asset.assetType === "rail_station" || asset.assetType === "rail_segment");
  const events = overview.events.filter((event) => event.eventType.startsWith("rail_") || /rail|srt|drt|crossing/i.test(event.sourceId));
  const cases = buildRailCasesFromRealData(crossings, events);
  const usedSourceIds = new Set([...crossings.map((asset) => asset.sourceId), ...events.map((event) => event.sourceId)]);
  const sources = overview.sources.filter((source) => usedSourceIds.has(source.id) || sourceLooksRail(source));
  const sourceHealth = overview.sourceHealth.filter((health) => usedSourceIds.has(health.sourceId) || /rail|srt|drt|crossing/i.test(health.sourceId));

  return {
    generatedAt: overview.generatedAt,
    region: "Thailand SRT level crossings spatial read model",
    sources,
    sourceHealth,
    crossings,
    events,
...
export async function getReadModelOntology(filters: {
  ...
  if (shouldThrowReadModelFailure()) throw new Error("Supabase smart_city ontology read model returned no usable rows.");
...
export async function getReadModelInsights(filters: InsightFilter): Promise<{
  ...
  if (shouldThrowReadModelFailure()) throw new Error("Supabase smart_city insights read model returned no usable rows.");
  const [overview, railOverview] = await Promise.all([getThailandTransportOverview(), getRailSafetyOverview()]);
  const model = buildOntologyReadModel({ overview, railOverview });
