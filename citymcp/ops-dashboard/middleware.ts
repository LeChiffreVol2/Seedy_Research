...
function basicAuthEnabled() {
  return Boolean(
    (process.env.OPS_DASHBOARD_BASIC_AUTH_USER && process.env.OPS_DASHBOARD_BASIC_AUTH_PASSWORD) ||
      process.env.OPS_RBAC_POLICY_JSON?.trim(),
  );
}
...
function isAuthorized(request: NextRequest) {
  ...
  return Boolean(resolveOpsBasicAuth(username, password));
}
