...
import {
  ...
  sourceIdsEligibleForMissingRowStaleMarking,
...
test("ingest refresh rejects missing or wrong secret", () => {
  ...
  assert.equal(isAuthorizedIngestRequest(new Request("http://localhost/api/ops/ingest/refresh", { method: "POST" })), false);
  assert.equal(
    isAuthorizedIngestRequest(new Request("http://localhost/api/ops/ingest/refresh", { method: "POST", headers: { authorization: "Bearer wrong" } })),
    false,
  );
  assert.equal(
    isAuthorizedIngestRequest(new Request("http://localhost/api/ops/ingest/refresh", { method: "POST", headers: { authorization: "Bearer unit-secret" } })),
    true,
  );
  assert.equal(
    isAuthorizedIngestRequest(new Request("http://localhost/api/ops/ingest/refresh", { method: "POST", headers: { authorization: "Bearer cron-secret" } })),
    false,
  );
  assert.equal(
    isAuthorizedIngestRequest(new Request("http://localhost/api/ops/ingest/refresh", { method: "GET", headers: { authorization: "Bearer cron-secret" } })),
    true,
  );
  assert.equal(
    isAuthorizedIngestRequest(new Request("http://localhost/api/ops/ingest/refresh", { method: "GET", headers: { authorization: "Bearer unit-secret" } })),
    false,
  );
...
test("degraded sources keep prior read-model rows out of missing-row stale marking", () => {
  assert.deepEqual(
    sourceIdsEligibleForMissingRowStaleMarking([
      { sourceId: "ok-source", status: "ok" },
      { sourceId: "degraded-source", status: "degraded" },
      { sourceId: "stale-source", status: "stale" },
      { sourceId: "offline-source", status: "offline" },
      { sourceId: "config-source", status: "needs_config" },
    ]),
    ["ok-source", "stale-source"],
  );
});
