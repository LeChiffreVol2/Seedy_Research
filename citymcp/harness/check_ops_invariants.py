...
OPS_ROOT = ROOT / "citymcp" / "ops-dashboard"
...
def check_ops_server_secret_boundary() -> Check:
    if not OPS_ROOT.exists():
        return Check("ops_server_secret_boundary", "fail", "citymcp/ops-dashboard is missing.", "Restore the isolated CityMCP app.")
...
def check_ops_ingest_auth_guard() -> Check:
    ...
    overview = OPS_API_ROOT / "overview" / "route.ts"
    if not route.exists() or not auth.exists() or not middleware.exists() or not overview.exists():
        return Check("ops_ingest_auth_guard", "fail", "Ingest route, auth helper, middleware, or overview route is missing.", "Restore the guarded ingest refresh endpoint.")

    route_text = read(route)
    auth_text = read(auth)
    middleware_text = read(middleware)
    overview_text = read(overview)
    required_route = [
    ...
    forbidden_overview = [needle for needle in ["ensureFreshSpatialCoreReadModel", "refreshSpatialCoreReadModel"] if needle in overview_text]
    if missing:
        return Check(
    ...
    if forbidden_overview:
        return Check(
            "ops_ingest_auth_guard",
            "fail",
            f"overview route performs ingest refresh: {forbidden_overview}",
            "Keep GET /api/ops/overview read-only; refresh must stay behind authenticated ingest route or cron.",
        )
...
def check_ops_ingest_cron_cadence() -> Check:
    ...
    try:
        config = json.loads(read(vercel_config))
    except json.JSONDecodeError as exc:
        return Check("ops_ingest_cron_cadence", "fail", f"Invalid vercel.json: {exc}", "Fix vercel.json syntax.")

    refresh_jobs = [job for job in config.get("crons", []) if job.get("path") == "/api/ops/ingest/refresh"]
    if not refresh_jobs:
...
def check_ops_action_record_research_guard() -> Check:
    ...
    action_log_route = OPS_API_ROOT / "actions" / "log" / "route.ts"
    ...
    missing_files = [str(path.relative_to(ROOT)) for path in [action_route, action_log_route, action_gate, action_log, research_gate, evidence_gate, rail_execute] if not path.exists()]
    if missing_files:
    ...
    action_log_route_text = read(action_log_route)
    ...
    required = [
        ...
        (action_log_route_text, "getOpsActor"),
        (action_log_route_text, "requireOpsPermission"),
        (action_log_route_text, '"read.ops"'),
        ...
        (rail_execute_text, 'requireOpsPermission(actor, "read.ops")'),
        (rail_execute_text, "Client-supplied railCase/proposal payloads are not trusted."),
        (gate_text, "getReadModelInsights"),
        (gate_text, "Insight must exist in the current read model."),
        (gate_text, "objectIds must belong to the selected server-side insight."),
    ...
    forbidden = []
    if "insight?: SmartCityInsight" in gate_text:
        forbidden.append("research-gate trusts client-supplied SmartCityInsight")
...
def check_ops_spatial_rpc_read_model_presence() -> Check:
    ...
    required_read_model_functions = [
        ...
        "eligibleSourceIds",
        "isEligibleForLayers",
        "isEligibleForInsights",
        "getRailSafetyOverview({ allowReadModel: false })",
        "shouldThrowReadModelFailure",
        "rpcRows",
        ...
        "Supabase smart_city overview read model returned no usable rows.",
        "Supabase smart_city ontology read model returned no usable rows.",
        "Supabase smart_city insights read model returned no usable rows.",
    ]
    ...
    required_sql_markers = [
        ...
        "revoke all on function public.smart_city_get_layer_features",
        "grant execute on function public.smart_city_get_layer_features",
        "revoke all on function public.smart_city_get_layer_feature_stats",
        "grant execute on function public.smart_city_get_layer_feature_stats",
        "revoke all on function public.smart_city_get_layer_features_page",
        "grant execute on function public.smart_city_get_layer_features_page",
        "revoke all on function public.smart_city_get_layer_mvt",
        "grant execute on function public.smart_city_get_layer_mvt",
        "revoke all on function public.smart_city_begin_ingest_run",
        "grant execute on function public.smart_city_begin_ingest_run",
        "revoke all on function public.smart_city_mark_missing_rows_stale",
        "grant execute on function public.smart_city_mark_missing_rows_stale",
        "revoke all on function public.smart_city_finish_ingest_run",
        "grant execute on function public.smart_city_finish_ingest_run",
        "revoke all on function public.smart_city_transition_action_record",
        "grant execute on function public.smart_city_transition_action_record",
...
def check_ops_agentic_audit_lifecycle_presence() -> Check:
    required_files = [
        OPS_ROOT / "lib" / "ops-auth-shared.ts",
        OPS_ROOT / "lib" / "ops-auth.ts",
        ...
        OPS_API_ROOT / "research-gate" / "route.ts",
        OPS_API_ROOT / "research" / "route.ts",
        OPS_API_ROOT / "rail" / "research" / "route.ts",
        OPS_API_ROOT / "analyst" / "route.ts",
    ...
    required_markers = [
        (OPS_ROOT / "lib" / "ops-auth-shared.ts", "OPS_RBAC_POLICY_JSON"),
        (OPS_ROOT / "lib" / "ops-auth-shared.ts", '"record.action"'),
        (OPS_ROOT / "lib" / "ops-auth.ts", "resolveOpsBasicAuth"),
        ...
        (OPS_API_ROOT / "commands" / "execute" / "route.ts", "run.research_gate"),
        (OPS_API_ROOT / "research-gate" / "route.ts", "run.research_gate"),
        (OPS_API_ROOT / "research" / "route.ts", "run.research_gate"),
        (OPS_API_ROOT / "rail" / "research" / "route.ts", "run.research_gate"),
        (OPS_API_ROOT / "analyst" / "route.ts", "run.research_gate"),
