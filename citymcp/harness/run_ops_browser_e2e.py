...
from run_ops_api_contracts import auth_pair, load_ops_env
...
def browser_script() -> str:
    return textwrap.dedent(
        ...
        await page.locator(".opsShell").waitFor({ state: "visible", timeout: 30000 });
        ...
        await check("source_dock_tab_clickable", async () => {
          const tab = page.locator('.dockTabs button[aria-label="Sources"]').first();
          await tab.waitFor({ state: "visible", timeout: 10000 });
          await tab.click({ timeout: 10000 });
          await page.waitForFunction(() => {
            const tab = document.querySelector('.dockTabs button[aria-label="Sources"]');
            const dock = document.querySelector(".opsDock");
            return tab?.getAttribute("aria-pressed") === "true" && /Source status/i.test(dock?.textContent || "");
          }, null, { timeout: 5000 });
          return true;
        });
        await check("analyst_tab_clickable", async () => {
          const tab = page.locator('.dockTabs button[aria-label="CivilMCP analyst"]').first();
          await tab.waitFor({ state: "visible", timeout: 10000 });
          await tab.click({ timeout: 10000 });
          await page.waitForFunction(() => {
            const tab = document.querySelector('.dockTabs button[aria-label="CivilMCP analyst"]');
            const dock = document.querySelector(".opsDock");
            return tab?.getAttribute("aria-pressed") === "true" && /CivilMCP Analyst/i.test(dock?.textContent || "");
          }, null, { timeout: 5000 });
          return true;
        });
...
def run_browser(base_url: str, env: dict[str, str], strict: bool) -> list[Check]:
    with tempfile.TemporaryDirectory(prefix="citymcp-browser-e2e-") as temp_dir:
        script_path = Path(temp_dir) / "ops-browser-e2e.cjs"
        script_path.write_text(browser_script(), encoding="utf-8")
        runtime_env = dict(env)
        runtime_env["OPS_E2E_BASE_URL"] = base_url
        runtime_env["NODE_PATH"] = str(ROOT / "citymcp" / "ops-dashboard" / "node_modules")
        user, password = auth_pair(env)
        if user and password:
            runtime_env["OPS_DASHBOARD_BASIC_AUTH_USER"] = user
            runtime_env["OPS_DASHBOARD_BASIC_AUTH_PASSWORD"] = password
        result = subprocess.run(
            ["node", str(script_path)],
            cwd=str(ROOT / "citymcp" / "ops-dashboard"),
            env=runtime_env,
    ...
    if result.returncode != 0 and ("Cannot find package 'playwright'" in output or "Cannot find module 'playwright'" in output):
        return [
            Check(
                ...
                "Install @playwright/test or playwright in citymcp/ops-dashboard and run playwright install chromium before strict browser E2E.",
            )
        ]
...
def main() -> None:
    ...
    parser.add_argument("--base-url", help="Ops dashboard base URL. Defaults to OPS_E2E_BASE_URL, OPS_DASHBOARD_URL, or localhost:3001.")
