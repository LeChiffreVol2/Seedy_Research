...
- Coverage Ledger: `/api/research-feed` returns dated provider rows that keep searchable metadata, page-citable evidence, native reader assets, source-hosted links, endpoint coverage, rights state, and not-yet-connected providers separate. Malformed provider IDs fail validation, unknown well-formed IDs return no records, and filtered counts never imply national completeness.
