...
function fallbackResponse(objectIds: string[], insight: SmartCityInsight | undefined, reason: string): ResearchGateResponse {
  const response: ResearchGateResponse = {
    ...
    findings: [
      {
        kind: "live_data",
        title: "Live data",
        summary: insight ? `${insight.title}: ${insight.whyNow}` : "No selected insight was supplied.",
        evidence: [],
      },
      {
        kind: "mcp_research",
        title: "MCP research",
        summary: "CivilMCP did not return cited evidence, so no ops action can be recorded.",
        evidence: [],
      },
    ],
    recommendedActions: [],
    ...
    limitations: [
      reason,
      "CivilMCP is read-only in this workflow.",
      "No action is recorded without cited MCP evidence.",
      "No write was made to civil_* tables or external field systems.",
    ],
  };
  if (insight) {
...
export async function POST(request: Request) {
  ...
  const objectIds = [...new Set(body.objectIds ?? body.insight?.sourceObjectIds ?? [])];
  const insight = body.insight;

  if (!insight || objectIds.length === 0) {
    return NextResponse.json({ error: "insight and objectIds are required" }, { status: 422 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      fallbackResponse(objectIds, insight, error instanceof Error ? error.message : "CivilMCP research failed."),
